...
function addSubscription(subscription, lat, lon) {
  ...
  console.log(`[Notifications] ✅ Подписка добавлена (lat: ${lat.toFixed(2)}, lon: ${lon.toFixed(2)}). Всего подписчиков: ${subscribers.size}`);
  console.log(`[Notifications] Endpoint: ${key.substring(0, 50)}...`);
  return true;
}
 ...
 * @returns {boolean} true если подписка найдена, false если нет
 */
...
/**
 * ТЕСТОВАЯ функция — отправить уведомление всем подписчикам
 * (для проверки перед праздником, потом удалить)
 */
async function sendTestNotification() {
  console.log(`[Notifications TEST] Запрос на тест. Подписчиков в базе: ${subscribers.size}`);
  
  if (subscribers.size === 0) {
    console.warn('[Notifications TEST] ❌ Нет подписчиков!');
    return { success: false, error: 'Нет подписчиков' };
  }
  if (!VAPID_PUBLIC || !VAPID_PRIVATE) {
    console.warn('[Notifications TEST] ❌ VAPID ключи не настроены!');
    return { success: false, error: 'VAPID ключи не настроены' };
  }

  console.log(`[Notifications TEST] 🚀 Отправка тестового уведомления ${subscribers.size} подписчикам...`);
  let sent = 0;
  let failed = 0;

  for (const [key, sub] of subscribers) {
    console.log(`[Notifications TEST] Отправка на: ${key.substring(0, 50)}...`);
    const success = await sendPush(sub.subscription, {
      title: '🌈 Тест уведомлений',
      body: 'Это тестовое уведомление. Если вы его видите — всё работает отлично!',
      data: { test: true }
    });
    if (success) {
      sent++;
      console.log(`[Notifications TEST] ✅ Успешно отправлено`);
    } else {
      failed++;
      console.log(`[Notifications TEST] ❌ Ошибка отправки`);
    }
  }

  console.log(`[Notifications TEST] 📊 Итого: ${sent} успешно, ${failed} ошибок`);
