...
/**
 * Отправить произвольное уведомление всем подписчикам
 */
async function broadcastNotification(title, body) {
  ...
  for (const [key, sub] of subscribers) {
    try {
      ...
      console.error(`[Notifications] Ошибка отправки broadcast:`, e.message);
    }
  }

  console.log(`[Notifications] Broadcast отправлен: ${sent}/${total}`);
  return { total, sent };
}
